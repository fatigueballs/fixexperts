...
class AdminViewUsersActivity : AppCompatActivity() {
    ...
    private lateinit var userKeyMap: MutableMap<String, String> // Map username to Firebase Key
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        adapter = AdminUserAdapter(this, userList) { user, username ->
            val userId = userKeyMap[username]
            if (userId != null) {
                val intent = Intent(this, AdminRequestHistoryActivity::class.java)
                intent.putExtra("USER_ID", userId) // Pass user's Firebase key
                intent.putExtra("USER_NAME", user.username)
                intent.putExtra("QUERY_CHILD", "userId") // Tell history activity to query by userId
            ...
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                for (uSnap in snapshot.children) {
                    ...
                    if (user != null && userKey != null) {
                        userList.add(user)
                        userKeyMap[user.username] = userKey // Store the key against the username
                    }
                }
